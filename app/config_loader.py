...
def load_config():
    ...
    try:
        ...
        if isinstance(raw_config.get('agent_prompt_template'), list):
            ...
            # Note: {tools} and {tool_names} are handled by Langchain, not formatted here
            CONFIG['agent_system_prompt'] = agent_template.format(**raw_config)
        else:
            CONFIG['agent_system_prompt'] = "You are a helpful assistant." # Fallback
            logger.error("agent_prompt_template is not a list in config.json")

        # Ensure required keys are present after formatting
        required_keys = [
            'initial_message',
        ...
        for key in required_keys:
             if key not in CONFIG:
                  ...
                  if key == 'agent_system_prompt': CONFIG[key] = "You are a helpful assistant."
        ...
        CONFIG = {'agent_system_prompt': "You are a helpful assistant.", 'calendar_tool_description': "Calendar tool.", 'initial_message': "Error loading config."}
